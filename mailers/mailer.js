...
const memberDonation = async (donationData, memberData) => {
  try {
    ...
    console.log("error in sending member donation mail", err);
  }
};
    ...
    fs.readFile(ejsPath, "utf-8", async (err, content) => {
      ...
      await page
        ...
        .then(async () => {
          console.log("file genreated successfully");
          if (sendMail) {
            await memberDonation(donationData, memberData);
          }
        });
      await browser.close();
      console.log("file generated");
...
const deadlineDonationMailer = async (member) => {
  try {
    console.log(member);
